...
__force_inline static void i8253_write(const uint16_t port_number, const uint8_t data) {
    if (port_number <= 0x42) {
        ...
        if (channel_index == 0) {
            ...
            // PC speaker. The reload value is the frequency divisor off
            // the PIT's 1.193182 MHz input; the waveform itself is
            // generated by the mixer on core 0, so all that happens here
            // is that the number is handed over.
            //
            // It used to reprogram the PWM wrap register to the reload
            // value, which made the audio resolution and the guest's
            // chosen pitch the same number -- eleven bits at a typical
            // reload of 1193, seven at a high-pitched 100.
            audio_speaker_divisor(channel->reload_value);
            audio_speaker_gate((port61 & 3) == 3);
        }
    } else {
